...
const fetchHistory = async () => {
  try {
    ...
    const res = await axios.get(`http://127.0.0.1:8000/api/donneur/historique`, {
...
const confirmDelete = async (id) => {
  ...
  if (result.isConfirmed) {
    try {
      const token = localStorage.getItem('token')
      await axios.delete(`http://127.0.0.1:8000/api/donneur/reserver/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
...
const handleEdit = async (don) => {
  ...
  if (newTime) {
    try {
      const token = localStorage.getItem('token')
      await axios.put(`http://127.0.0.1:8000/api/donneur/reserver/${don.id}`, 
        { heure_rdv: newTime },
        { headers: { Authorization: `Bearer ${token}` } }
      )
