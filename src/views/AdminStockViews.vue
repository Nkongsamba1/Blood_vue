    ...
    <div v-else class="flex-1 flex flex-col min-w-0 animate-fade-in">
      ...
      <header class="h-24 lg:h-28 bg-white/80 backdrop-blur-md flex items-center justify-between px-6 lg:px-12 border-b border-slate-100 sticky top-0 z-20">
        <div class="ml-12 lg:ml-0">
          ...
          <p class="text-[8px] lg:text-[10px] font-bold text-red-600 uppercase tracking-[0.2em] mt-1">Monitoring des réserves en litres</p>
        </div>
      ...
      <main class="flex-1 p-6 lg:p-12 bg-[#fafafa]">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 lg:gap-10">
          ...
          <div v-for="(groupe, index) in listeGroupes" :key="groupe" 
            ...
            <div class="absolute top-6 right-6 lg:top-8 lg:right-8 px-3 py-1 rounded-full text-[9px] font-black italic uppercase"
            ...
            <div class="text-center space-y-1 mb-6">
              <p class="text-[8px] lg:text-[9px] font-black text-slate-400 uppercase tracking-widest">Volume en stock</p>
            ...
            <div v-if="getPercentage(groupe) < 25" class="py-2.5 lg:py-3 bg-red-600 text-white text-[8px] lg:text-[9px] font-black uppercase text-center rounded-xl lg:rounded-2xl animate-bounce shadow-lg shadow-red-200">
              ⚠️ Réserve Critique
            </div>
        ...
        <div class="mt-12 lg:mt-16 bg-[#111] rounded-[2.5rem] lg:rounded-[4rem] p-8 lg:p-12 flex flex-col md:flex-row items-center justify-between shadow-2xl relative overflow-hidden animate-slide-up" style="animation-delay: 0.8s">
          <div class="relative z-10 text-center md:text-left">
            ...
            <p class="text-slate-500 text-[8px] lg:text-[10px] font-bold uppercase tracking-[0.2em]">Ciblez les groupes critiques via une campagne de don.</p>
          </div>
          
          <router-link 
            to="/NouvelleCampagne" 
            class="mt-6 md:mt-0 bg-red-600 text-white px-8 lg:px-10 py-4 lg:py-5 rounded-2xl lg:rounded-[2rem] font-black uppercase italic tracking-widest text-[10px] lg:text-xs shadow-xl hover:bg-white hover:text-red-600 transition-all z-10"
          >
            Nouvelle Campagne 🚀
          </router-link>
...
// On définit le 100% à 10 litres (10000 ml) par groupe pour l'hôpital
...
const fetchStocks = async () => {
  ...
  try {
    ...
    // Délai pour laisser l'animation de chargement respirer
...
const getTotalVolumeMl = (nom) => {
  ...
  return item ? item.total_ml : 0;
};

const getPercentage = (nom) => {
  const total = getTotalVolumeMl(nom);
  return Math.min(Math.round((total / SEUIL_SECURITE_ML) * 100), 100);
};

const getBloodColorClass = (nom) => {
  const p = getPercentage(nom);
  if (p < 25) return 'bg-gradient-to-t from-red-600 to-red-400 animate-pulse'; 
  if (p < 60) return 'bg-gradient-to-t from-orange-500 to-orange-300'; 
  return 'bg-gradient-to-t from-red-800 to-red-600'; 
};
...
/* Scroll bar personnalisée pour le look admin */
