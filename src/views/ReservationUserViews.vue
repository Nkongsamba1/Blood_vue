...
const init = async () => {
  ...
  try {
    ...
    const resElig = await axios.get(`${import.meta.env.VITE_API_URL}/api/donneur/check-eligibilite`, { headers })
    eligibilite.value = resElig.data

    // 2. Si éligible, charger les campagnes
    if (eligibilite.value.eligible) {
      const resCamp = await axios.get(`${import.meta.env.VITE_API_URL}/api/campagnes/disponibles`, { headers })
...
const handleReserve = async () => {
  ...
  if (result.isConfirmed) {
    try {
      const token = localStorage.getItem('token')
      await axios.post('${import.meta.env.VITE_API_URL}/api/donneur/reserver', {
