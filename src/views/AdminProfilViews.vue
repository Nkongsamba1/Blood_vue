              ...
              <div class="relative z-10">
                <div class="relative w-32 h-32 lg:w-36 lg:h-36 mx-auto mb-6 cursor-pointer" @click="fileInput.click()">
                  <div class="w-full h-full bg-red-600 rounded-[2rem] lg:rounded-[2.5rem] flex items-center justify-center text-white text-5xl font-black italic shadow-xl border-4 border-white/10 overflow-hidden transition-transform group-hover:scale-95">
                    <img v-if="user.photo" :src="`http://127.0.0.1:8000/storage/${user.photo}`" class="w-full h-full object-cover">
...
const updateProfile = async () => {
  try {
    const token = localStorage.getItem('token');
    await axios.put(`http://127.0.0.1:8000/api/users/${user.value.utilisateur_id}`, form.value, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
const handleImageUpload = async (event) => {
  ...
  try {
    const token = localStorage.getItem('token');
    const response = await axios.post('http://127.0.0.1:8000/api/admin/profil/photo', formData, {
      headers: { 
...
const handleUpdatePassword = async () => {
  ...
  try {
    const token = localStorage.getItem('token');
    await axios.put(`http://127.0.0.1:8000/api/admin/password/update`, passwords, {
      headers: { Authorization: `Bearer ${token}` }
    });
