<template>
  <button 
    @click="isOpen = !isOpen" 
    class="lg:hidden fixed top-8 left-6 z-[60] bg-[#111] p-3 rounded-xl shadow-2xl border border-white/10 text-white"
  >
  ...
  >
    ...
    <div class="p-10 border-b border-white/5 flex items-center gap-4 group">
      ...
      <div>
        ...
        <p class="text-[8px] font-bold text-red-500 uppercase tracking-[0.3em] mt-1">Management Système</p>
      </div>
    </div>
    ...
    <nav class="p-6 space-y-2 flex-1">
      ...
      <div class="space-y-1">
        <button @click="toggleMenu('stocks')" :class="{'bg-white/5 text-white': menus.stocks}" class="nav-link w-full group">
          ...
          <span class="text-[10px] transition-transform duration-300" :class="{'rotate-180': menus.stocks}">▼</span>
        </button>
      ...
      <div class="space-y-1">
        <button @click="toggleMenu('users')" :class="{'bg-white/5 text-white': menus.users}" class="nav-link w-full group">
          ...
          <span class="text-[10px] transition-transform duration-300" :class="{'rotate-180': menus.users}">▼</span>
        </button>
        <transition name="slide">
      ...
      <div class="pt-8 pb-2">
        <p class="text-[9px] font-black text-white/20 uppercase tracking-[0.2em] px-8 italic">Personnel</p>
      </div>
    ...
    <div class="p-6 border-t border-white/5">
      <button @click="handleLogout" class="flex items-center gap-4 w-full px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-red-500 hover:bg-red-600 hover:text-white transition-all duration-300">
        <span>🚪</span>
...
const handleLogout = () => {
  Swal.fire({
    title: 'Déconnexion ?',
    text: "Êtes-vous sûr de vouloir quitter la session admin ?",
    icon: 'warning',
    showCancelButton: true,
    confirmButtonColor: '#dc2626', // Rouge (red-600)
...
.router-link-active {
  @apply bg-red-600 text-white shadow-xl shadow-red-900/40 !important;
}

.sub-nav-link {
  @apply block py-3 text-[10px] font-bold uppercase tracking-widest text-white/30 hover:text-red-500 transition-colors border-l border-white/5 pl-4 hover:border-red-500 cursor-pointer;
}

.sub-nav-link.router-link-active {
  @apply bg-transparent text-red-500 border-red-500 shadow-none !important;
}
