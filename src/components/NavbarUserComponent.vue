  ...
  <nav class="bg-slate-900/95 backdrop-blur-md text-white shadow-2xl sticky top-0 z-50 border-b border-white/5">
    <div class="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
      ...
      <router-link to="/Dash_user" class="flex items-center gap-3 group">
        <div class="bg-gradient-to-br from-[#A62639] to-red-700 text-white w-10 h-10 flex items-center justify-center rounded-xl font-black shadow-lg shadow-red-500/20 transition-all duration-500 group-hover:rotate-[360deg] group-hover:scale-110">
          KG
        </div>
        <div class="flex flex-col leading-none">
          <span class="font-black text-xl tracking-tighter uppercase italic text-white">Saint-Raphaël</span>
          <span class="text-[8px] font-bold uppercase tracking-[0.4em] text-[#A62639]">Blood Care</span>
        </div>
      </router-link>
      ...
      <div class="hidden md:flex items-center border-l border-white/10 pl-6 ml-2">
        <button 
          @click="handleLogout"
          class="flex items-center gap-2 bg-white/5 hover:bg-[#A62639] group px-5 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all duration-300 border border-white/10 hover:border-red-500"
        >
          <span class="text-[#A62639] group-hover:text-white transition-colors italic">Quitter</span>
      ...
      <button @click="isMenuOpen = !isMenuOpen" class="md:hidden w-10 h-10 flex flex-col items-center justify-center gap-1.5 bg-white/5 rounded-lg border border-white/10 outline-none">
        <span :class="['block w-5 h-0.5 bg-white transition-all duration-300', isMenuOpen ? 'rotate-45 translate-y-2' : '']"></span>
        <span :class="['block w-3 h-0.5 bg-[#A62639] transition-all duration-300', isMenuOpen ? 'opacity-0' : '']"></span>
        <span :class="['block w-5 h-0.5 bg-white transition-all duration-300', isMenuOpen ? '-rotate-45 -translate-y-2' : '']"></span>
      </button>
    </div>
      ...
      <div v-if="isMenuOpen" class="md:hidden bg-slate-900 border-t border-white/5 absolute w-full shadow-2xl overflow-hidden">
        <div class="px-6 py-8 flex flex-col gap-4">
          ...
          <div class="pt-6 mt-2 border-t border-white/5">
            <button 
              @click="handleLogout"
              class="w-full bg-[#A62639] hover:bg-red-700 text-white py-4 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-lg shadow-red-600/20 transition-all active:scale-95"
            >
              Déconnexion
            </button>
          </div>
...
const handleLogout = async () => {
  ...
  if (result.isConfirmed) {
    try {
      // Utilisation de l'instance centralisée pour le logout
